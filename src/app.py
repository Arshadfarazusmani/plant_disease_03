...
def model_prediction(test_image):
    """Predicts the disease from an image and returns the prediction."""
...
def main():
    ...
    with st.sidebar:
        ...
        if st.session_state.user:
            ...
            app_mode = st.radio("Select Page", ["Home", "Disease Recognition", "About", "Logout", "Profile"])
        else:
            app_mode = st.radio("Select Page", ["Home", "Login", "Signup", "Disease Recognition", "About"])
    ...
    # Disease Recognition Page
        ...
        if not st.session_state.user:
            st.warning("Please log in to use the Disease Recognition feature.")
        else:
            ...
            if test_image is not None:
                ...
                if st.button("Predict"):
                    ...
                    image_id = save_image(user_id, test_image.name)
                    if image_id is not None:
                        ...
                        predicted_disease_name = class_name[predicted_class_index]
                        disease_id = get_disease_id(predicted_disease_name)

                        if disease_id:
                            save_prediction(image_id, disease_id, confidence_score)
                            diagnosis_info = {
                ...
                'Tomato___healthy': "The tomato appears healthy. Ensure proper staking and irrigation."
                            ...
                            prediction_label = class_name[predicted_class_index]
                            diagnosis = diagnosis_info.get(prediction_label,
                                                            "Diagnosis information not available.")
                            st.success(
                                f"**Prediction:** {prediction_label}  (Confidence: {confidence_score:.2f})\n\n**Diagnosis:** {diagnosis}")
                        else:
                            st.error("Disease not found in database. Please update the database.")
                    else:
                        st.error("Failed to save image.  Prediction cancelled.")
        ...
        if not st.session_state.user:
            ...
            try:
                ...
                for image in user_images:
                    ...
                    prediction = class_name[ predictions] # changed image_id
                    if prediction:
